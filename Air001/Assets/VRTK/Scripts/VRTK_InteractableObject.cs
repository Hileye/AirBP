...
namespace VRTK
{
    ...
    public class VRTK_InteractableObject : MonoBehaviour
    {
        ...
        public float onGrabCollisionDelay = .5f;
        ...
        private GameObject lastControllerPicked;
        ...
        public virtual void Grabbed(GameObject currentGrabbingObject)
        {
            ...
            ScaleGrab(currentGrabbingObject);
        }
        ...
        public virtual void Ungrabbed(GameObject previousGrabbingObject)
        {
            ...
            ScaleRelease(previousGrabbingObject);
        }

        private void ScaleGrab(GameObject currentGrabbingObject)
        {
            // If the block is grabbed, scale its collider down so that it's easier to move the block around.
            gameObject.GetComponent<BoxCollider>().size = new Vector3(0.8f, 0.8f, 0.8f);

            // Scale the controller's collider up in order to keep holding the block.
            BoxCollider controllerCollider = currentGrabbingObject.GetComponent<BoxCollider>();
            controllerCollider.center = new Vector3(0f, 0f, 0.445f);
            controllerCollider.size = new Vector3(0.05f, 0.05f, 0.25f);
        }

        private void ScaleRelease(GameObject previousGrabbingObject)
        {
            // If the block is not grabbed, return it to its origional scale.
            gameObject.GetComponent<BoxCollider>().size = new Vector3(1f, 1f, 1f);

            BoxCollider controllerCollider = previousGrabbingObject.GetComponent<BoxCollider>();
            controllerCollider.center = new Vector3(0f, 0f, 0.345f);
            controllerCollider.size = new Vector3(0.05f, 0.05f, 0.05f);
        }
        ...
        {
            //detachThreshold = detachThresholdInspector;
            canDetach = true;
